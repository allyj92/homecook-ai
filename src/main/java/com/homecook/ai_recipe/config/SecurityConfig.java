    ...
    SecurityFilterChain security(HttpSecurity http) throws Exception {
        http
                ...
                .exceptionHandling(ex -> ex
                        // ★ /api/** 에서는 로그인 페이지로 리다이렉트하지 말고 401만 주기
                ...
                .oauth2Login(oauth -> oauth
                        .successHandler((req, res, auth) -> res.sendRedirect(frontBase + "/"))
                        .failureHandler((req, res, ex) -> {
                            ex.printStackTrace();
                            res.sendRedirect(frontBase + "/login-signup?error=" + ex.getClass().getSimpleName());
                        })
                )
                .logout(lo -> lo.logoutUrl("/api/auth/logout").logoutSuccessHandler((req, res, auth) -> res.setStatus(204)))
