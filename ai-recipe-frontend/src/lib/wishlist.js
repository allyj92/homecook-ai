...
import { logActivity } from './activity'; // 활동 로그
...
/* ─────────────────────────────────────────
   계정별 캐시 격리(카카오/네이버/구글)용 서명 & 캐시버스터
   - CDN/리버스프록시가 쿠키 무시하고 URL만 캐싱할 때 대비
   - GET 요청에 uid:provider 서명과 타임스탬프를 붙여서
     사용자 간 응답이 섞이지 않도록 함
────────────────────────────────────────── */
...
function userSig() {
  const a = readAuthUser();
  if (!a) return 'guest';
  return `${a.uid}:${a.provider}`;
}
function withSig(url) {
  const sig = encodeURIComponent(userSig());
  const ts = Date.now();
  return `${url}${url.includes('?') ? '&' : '?'}__u=${sig}&__t=${ts}`;
}
...
export async function listFavorites() {
  const url = withSig(api('/api/me/favorites'));
...
export async function listFavoritesSimple(size = 3) {
  const url = withSig(api(`/api/me/favorites?page=0&size=${encodeURIComponent(size)}`));
  const res = await apiFetch(url, { cache: 'no-store' });
  return normalizeArrayResponse(res);
}
...
export async function listFavoritesPage({ page = 0, size = 12 } = {}) {
  const url = withSig(api(`/api/me/favorites?page=${encodeURIComponent(page)}&size=${encodeURIComponent(size)}`));
  const res = await apiFetch(url, { cache: 'no-store' });
  if (!res.ok) {
    const server = await safeJson(res);
    if (res.status === 401) throw new Error(pickServerMessage(server, '로그인이 필요합니다.'));
...
export async function removeFavorite(recipeId) {
  ...
  const url = withSig(api(`/api/me/favorites/${rid}`));
  const res = await apiFetch(url, { method: 'DELETE', headers: { 'X-User-Sig': userSig() } });
  if (!res.ok) {
    const server = await safeJson(res);
    if (res.status === 401) throw new Error(pickServerMessage(server, '로그인이 필요합니다.'));
